...
<template>
  <div class="min-h-screen bg-poster-beige text-prohib-black pb-20">
    ...
    <section class="bg-prohib-black text-white pt-32 pb-20 px-6 text-center rounded-b-[3rem] shadow-xl relative z-10 overflow-hidden">
      ...
      <h1 class="animate-fade-up text-[8vw] sm:text-4xl md:text-5xl lg:text-6xl font-black mb-6 leading-tight font-main uppercase">
        {{ t('urgencePage.heroTitle') }}<br>
        <span class="text-reg-green">{{ t('urgencePage.heroSubtitle') }}</span>
      </h1>
      <p class="animate-fade-up text-[4.5vw] sm:text-xl md:text-2xl opacity-90 max-w-4xl mx-auto font-body leading-relaxed text-gray-300" v-html="t('urgencePage.heroDesc')">
      </p>
    </section>
    ...
    <div class="max-w-4xl mx-auto px-6 mt-16 space-y-12">
      ...
      <!-- ALERTE BOX -->
      <div class="scroll-fade bg-red-50 border-l-[6px] border-error-red p-5 sm:p-8 rounded-xl shadow-md">
        <h3 class="text-error-red text-[5vw] sm:text-2xl font-bold mb-4 flex items-center font-main uppercase">
          <span class="mr-3 text-[7vw] sm:text-3xl">⚠️</span> {{ t('urgencePage.alertTitle') }}
        </h3>
      ...
      <div class="scroll-fade">
        <h2 class="text-[6.5vw] sm:text-3xl font-black font-main uppercase mb-4">{{ t('urgencePage.solutionTitle') }}</h2>
        ...
        <div class="flex flex-col md:flex-row bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
          <!-- Visual -->
          <div class="flex-1 bg-gray-50 flex flex-col justify-center items-center p-6 sm:p-10 relative border-b md:border-b-0 md:border-r border-gray-200">
          ...
          <!-- Content -->
          <div class="flex-[2] p-5 sm:p-8 md:p-10">
            ...
            <form class="flex flex-col sm:flex-row gap-3" @submit.prevent>
              ...
              <button type="submit" class="bg-reg-green text-white font-bold uppercase tracking-wider px-6 py-3 rounded-lg hover:bg-green-700 transition-colors shadow-md active:scale-95">{{ t('urgencePage.formBtn') }}</button>
            </form>
      ...
      <div class="scroll-fade pt-12">
        <h2 class="text-[6.5vw] sm:text-3xl font-black font-main text-center uppercase mb-4">{{ t('urgencePage.expertsTitle') }}</h2>
        <p class="text-center text-lg text-gray-600 max-w-2xl mx-auto mb-12 font-body">{{ t('urgencePage.expertsDesc') }}</p>
        
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div v-for="(expert, idx) in tm('urgencePage.experts')" :key="idx" class="bg-white rounded-2xl p-5 sm:p-6 text-center shadow-lg hover:-translate-y-2 active:scale-[0.98] transition-transform duration-300 border border-gray-100">
      ...
      <!-- FAQ SECTION -->
      <div class="scroll-fade bg-white rounded-3xl p-5 sm:p-8 md:p-12 shadow-xl border border-gray-100 mt-16">
        <h2 class="text-[6.5vw] sm:text-3xl font-black font-main text-center uppercase mb-10">{{ t('urgencePage.faqTitle') }}</h2>
        ...
        <div class="space-y-4">
          <div v-for="(faq, idx) in tm('urgencePage.faqs')" :key="idx" class="border-b border-gray-200 pb-4 last:border-0 last:pb-0">
            <button @click="toggleFaq(idx)" class="w-full flex justify-between items-center text-left py-4 focus:outline-none group">
              <span class="font-bold text-lg text-prohib-black group-hover:text-reg-green group-active:text-reg-green transition-colors pr-8 font-main">{{ faq.q }}</span>
      ...
      <div class="scroll-fade text-center py-16 border-t border-gray-200 mt-16">
        <h2 class="text-[7.5vw] sm:text-4xl font-black font-main uppercase mb-6">{{ t('urgencePage.joinTitle') }}</h2>
        <p class="text-xl text-gray-700 mb-10 leading-relaxed font-body" v-html="t('urgencePage.joinDesc')"></p>
        <a href="#" class="inline-block w-full sm:w-auto bg-error-red text-white px-[6vw] py-[4vw] sm:px-10 sm:py-5 rounded-full font-black uppercase tracking-widest text-[3.5vw] sm:text-lg whitespace-normal sm:whitespace-nowrap leading-snug hover:bg-red-700 transition-all duration-300 ease-out shadow-[0_10px_25px_-5px_rgba(211,47,47,0.5)] hover:shadow-[0_15px_35px_-5px_rgba(211,47,47,0.6)] hover:-translate-y-1 active:scale-95 active:shadow-inner">
