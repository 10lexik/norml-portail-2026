  ...
  <section ref="sectionRef" class="py-32 px-6 md:px-12 bg-prohib-black text-white relative rounded-t-[3rem] -mt-10 z-20 shadow-2xl">
    <div class="max-w-6xl mx-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        <component 
          :is="p.route ? 'router-link' : 'a'"
          v-for="(p, i) in projects" 
          :key="p.id" 
          :[p.route?'to':'href']="p.route || p.url"
          ...
          <div class="bg-prohib-black h-full rounded-[22px] p-8 relative overflow-hidden flex flex-col pointer-events-none">
            ...
            <div class="relative h-40 w-full mb-8 flex items-center justify-center">
              ...
              <div v-if="p.id === 'norml-trvknn'" class="relative w-full h-full flex items-center justify-center">
                <div class="absolute w-24 h-32 rounded-xl border border-white/20 bg-white/5 shadow-lg flex items-center justify-center card-3"><div class="w-10 h-1.5 bg-white/10 rounded-full"></div></div>
                <div class="absolute w-24 h-32 rounded-xl border border-white/20 bg-white/10 shadow-lg flex items-center justify-center card-2"><div class="w-10 h-1.5 bg-white/20 rounded-full"></div></div>
                <div class="absolute w-24 h-32 rounded-xl border-2 border-[#d4af37]/60 bg-[#141414] shadow-2xl flex items-center justify-center card-1">
                  <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#d4af37" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="question-mark drop-shadow-md"><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><path d="M12 17h.01"/></svg>
                  <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#4caf50" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="check-mark absolute opacity-0 scale-50 drop-shadow-lg"><polyline points="20 6 9 17 4 12"></polyline></svg>
                </div>
              </div>

              <!-- Globe -->
              <div v-else-if="p.id === 'norml-globe'" class="relative w-full h-full flex items-center justify-center perspective-[500px]">
                <div class="w-28 h-28 rounded-full border-2 border-white/20 flex items-center justify-center relative overflow-hidden globe-container">
                  ...
                  <div class="absolute w-2 h-2 bg-[#4caf50] rounded-full top-[30%] left-[65%] globe-ping shadow-[0_0_15px_2px_#4caf50]"></div>
                </div>
              </div>

              <!-- Urgence -->
              <div v-else-if="p.id === 'norml-urgence'" class="relative w-full h-full flex items-center justify-center">
                <div class="absolute w-24 h-24 rounded-full border border-white/20 bg-white/5 shadow-lg flex items-center justify-center urgence-3"></div>
                <div class="absolute w-24 h-24 rounded-full border border-white/20 bg-white/10 shadow-lg flex items-center justify-center urgence-2"></div>
                <div class="absolute w-24 h-24 rounded-full border-2 border-[#d32f2f]/60 bg-[#141414] shadow-2xl flex items-center justify-center urgence-1">
                  <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#d32f2f" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="drop-shadow-md"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                </div>
              </div>

              <!-- Ticket -->
              <div v-else-if="p.id === 'norml-ticket'" class="relative w-full h-full flex items-center justify-center">
                <div class="absolute w-20 h-28 rounded-sm border border-white/20 bg-white/5 shadow-lg flex flex-col items-center ticket-3 border-b-dotted border-b-4 border-b-white/20"><div class="w-10 h-1 bg-white/10 mt-4 rounded-full"></div></div>
                <div class="absolute w-20 h-28 rounded-sm border border-white/20 bg-white/10 shadow-lg flex flex-col items-center ticket-2 border-b-dotted border-b-4 border-b-white/20"><div class="w-10 h-1 bg-white/20 mt-4 rounded-full"></div></div>
                <div class="absolute w-20 h-28 rounded-sm border-2 border-[#2E8A42]/60 bg-[#141414] shadow-2xl flex flex-col items-center ticket-1 border-b-[4px] border-b-dotted border-b-[#2E8A42]/60">
                  <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#2E8A42" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="mt-4 drop-shadow-md"><path d="M16 16v-4"/><path d="M8 16v-4"/><path d="M12 16v-6"/><path d="M4 16v-2"/><path d="M20 16v-2"/></svg>
              ...
              <div v-else-if="p.id === 'norml-bilan'" class="relative w-full h-full flex items-center justify-center">
                <div class="absolute w-32 h-16 rounded-xl border border-white/20 bg-white/5 shadow-lg flex items-center justify-center bilan-3"></div>
                <div class="absolute w-32 h-16 rounded-xl border border-white/20 bg-white/10 shadow-lg flex items-center justify-center bilan-2"></div>
                <div class="absolute w-32 h-16 rounded-xl border-2 border-[#8bc34a]/60 bg-[#141414] shadow-2xl flex items-center justify-center bilan-1 overflow-hidden">
                  <div class="font-mono text-3xl font-black text-[#8bc34a] tracking-widest bilan-counter drop-shadow-[0_0_10px_rgba(139,195,74,0.5)]">
                    0:00
                  </div>
                </div>
              </div>
