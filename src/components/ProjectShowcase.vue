...
onMounted(() => {
  ...
  gsap.fromTo(cardsRef.value,
    ...
    { opacity: 1, y: 0, rotateX: 0, duration: 1, stagger: 0.2, ease: 'back.out(1.2)', scrollTrigger: { trigger: sectionRef.value, start: 'top 60%' } }
  )
          ...
          <div class="bg-prohib-black h-full rounded-[22px] p-8 relative overflow-hidden flex flex-col pointer-events-none">
            ...
            <div class="relative h-48 w-full mb-8 flex items-center justify-center">
              <div v-if="p.id === 'norml-trvknn'" class="relative w-full h-full flex items-center justify-center">
                <div class="absolute w-28 h-40 rounded-xl border border-white/20 bg-white/5 shadow-lg flex items-center justify-center card-3"><div class="w-14 h-2 bg-white/10 rounded-full"></div></div>
                <div class="absolute w-28 h-40 rounded-xl border border-white/20 bg-white/10 shadow-lg flex items-center justify-center card-2"><div class="w-14 h-2 bg-white/20 rounded-full"></div></div>
            ...
            <h3 class="text-3xl font-bold mb-4 font-main tracking-tight uppercase mt-auto" :style="{ color: p.color }">{{ $t(`projects.items.${p.key}.title`) }}</h3>
            <p class="text-gray-400 text-lg mb-8 font-body leading-relaxed">{{ $t(`projects.items.${p.key}.desc`) }}</p>
